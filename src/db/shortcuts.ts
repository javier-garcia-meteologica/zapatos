  ...
  if (Array.isArray(values) && values.length === 0) {
    ...
    const
      ...
      valuesSQL = Array.isArray(completedValues) ?
        mapWithSeparator(completedValues as GenericSQLStructure['Insertable'][], sql`, `, v => sql<GenericSQLStructure>`(${vals(v)})`) :
        sql<GenericSQLStructure>`(${vals(completedValues)})`,
...
const genericUpsert = function (
  ...
  const
    ...
    insertValuesSQL = mapWithSeparator(completedValues, sql`, `, v => sql<GenericSQLStructure>`(${vals(v)})`),
  ...
  const
    insertPart = sql<GenericSQLStructure>`INSERT INTO ${table} (${insertColsSQL}) VALUES ${insertValuesSQL}`,
